...
function [tao_out] = NE_mod(q,dq,dqA,ddq,KinPar,DynPar,g)
%#codegen
...
while count < n
    ...
    r_dd = extDH2Rdd(DH_ext(count,:),q(count));
...
while count > 0
    ...
    m_i  = DynPar(count).m;
    I_i  = DynPar(count).I-m_i*transpose(Skew(r_i_ici))*Skew(r_i_ici);
    ...
    if JT(count) == 1
        %prismatic
        tao(count) = transpose(f_i_i)*tR_im1_i*z_0 + Fv_i*dq(count)+...
                     Fc_i*sin(atan(1000*dq(count)))+ddq(count)*Kri^2*I_mi;
    elseif JT(count) == 0
        %rotoidal
        tao(count) = transpose(mu_i_i)*tR_im1_i*z_0 + Fv_i*dq(count)+...
                     Fc_i*sin(atan(1000*dq(count)))+ddq(count)*Kri^2*I_mi;
    end
