...
while k <= (N-1)
    ...
    Aio_i = Ro('z',-delta_pl)*Tr('z',-p_pl)*Tr('x',a_pl)*...
            Ro('x',alpha_pl)*Tr('z',n_pl)*Ro('z',phi_i)*Tr('z',-n_i);
    U_aux = pinv(Aio_i)*[r_io_com_i;1];
    Dyn_par(cntJ).r_com = U_aux(1:3);
    % inertia tensor
    R_aux = Aio_i(1:3,1:3); 
    %I_aux = % % as in the papser
    %transpose(R_aux)*(I_io_i-m_i*transpose(Skew(r_io_com_i))*Skew(r_io_com_i))*R_aux+m_i*transpose(Skew(U_aux(1:3)))*Skew(U_aux(1:3));
    I_aux = transpose(R_aux)*(I_io_i-m_i*transpose(Skew(r_io_com_i))*Skew(r_io_com_i))*R_aux; % as in the dissertation
