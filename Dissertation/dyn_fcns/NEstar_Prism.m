...
% Last update:  ---
% Last revision: 11-October-2016

%------------- BEGIN CODE --------------

function tau_e = NEstar_Prism(eDH, Dyn_Data, gravity_vec, arrayQ, arrayQa, isnumeric,B)
%#codgen
...
% ---Computing DH matrix and joint variables---
    ...
    switch isnumeric
        case 0
            ...
            theta_a_d_t = sym(zeros(nlink, 1));
            d_a_d_t = sym(zeros(nlink, 1));
    ...
    for k=1:nlink
        ...
        if(eDH(k,5)==0) % i.e: joint k is revolute
            ...
            acc(:,i)  = Rt*acc(:,i1)  + cross(gamma(:,i),p) + oomegap;   %%%       
             ...
             d_dzi=d_d_t(k)*zi;
             d_a_dzi= d_a_d_t(k)*zi;
             d_ddzi=d_dd_t(k)*zi;    
             
             omega(:,i)   = Rt*omega(:,i1);
             omega_a(:,i) = Rt*omega_a(:,i1);
             gamma(:,i) = Rt*gamma(:,i1);
             omegap_a = cross(omega_a(:,i),p);
             oomegap = cross(omega(:,i),omegap_a);
             
             omegad_dzi = cross(omega_a(:,i),d_dzi);
             omegad_a_dzi = cross(omega(:,i),d_a_dzi);
             
             acc(:,i)  = Rt*acc(:,i1) + cross(gamma(:,i),p) + oomegap ...
                 + omegad_dzi + omegad_a_dzi + d_ddzi; %%%
